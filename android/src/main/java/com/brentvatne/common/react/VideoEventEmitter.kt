    ...
    companion object {
        fun toMap(): MutableMap<String, Any> {
            ...
            EventTypes.entries.forEach { eventType ->
                result["top${eventType.eventName.removePrefix("on")}"] = mapOf("registrationName" to eventType.eventName)
            }
            return result
        }
    }
...
class VideoEventEmitter {
    ...
    private fun createEventClass(surfaceId: Int, viewTag: Int, event: EventTypes, setParams: ((WritableMap) -> Unit)? = null): Event<*> {
        val className = "top${event.eventName.replaceFirstChar(Char::titlecase)}"
        return object : Event<Event<*>>(surfaceId, viewTag) {
            override fun getEventName() = className
